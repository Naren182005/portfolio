...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const uniforms = {
      ...
      uImageResolution1: { value: new THREE.Vector2(1920, 1080) }, // Default Spiderman
      uImageResolution2: { value: new THREE.Vector2(1920, 1080) }  // Default Man
    };
    ...
    Promise.all([
      ...
      // Update image resolution based on actual dimensions
      if (tex1.image) {
        uniforms.uImageResolution1.value.set(tex1.image.width, tex1.image.height);
      }
      if (tex2.image) {
        uniforms.uImageResolution2.value.set(tex2.image.width, tex2.image.height);
      }
    ...
    const onMouseEnter = () => {
      ...
      // Expand mask fully to reveal image on hover perfectly
    ...
    const onMouseLeave = () => {
      ...
      // Revert radius back to spotlight size
      gsap.to(uniforms.uRadius, {
        value: 0.25,
        duration: 1.2,
        ease: "power3.out"
        ...
        >
          ...
          <div className="flex-1 max-w-lg lg:max-w-xl text-left">
            ...
            <h1 className="text-2xl md:text-3xl lg:text-[1rem] xl:text-[3.5rem] font-bold tracking-tighter drop-shadow-2xl leading-[1.05] font-sans">
              Building<br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-200 to-gray-500 font-serif italic font-light pr-2">Intelligent</span> Solutions<br />
              That Scale
            </h1>
          </div>
          ...
          <div className=" flex-1 max-w-md text-left md:text-right flex flex-col md:items-end">
            <p className="w-110 text-lg md:text-xl text-gray-300 drop-shadow-xl font-light tracking-wide leading-relaxed mb-8">
              From machine learning models to production-ready AI applications.
            </p>
